...
import pickle
from vibes.utils.help_functions import map_help_fun
...
def main():
    ...
    if pargs.grid == "no":
        storage_path = os.path.join(map_path, f"{pargs.loss}_c_{pargs.c}_tau_{pargs.tau}.pkl")
    
    else:
        storage_path = os.path.join(map_path, f"grid/{pargs.loss}/")
    ...
    if num_cores == 1:
        ...
        for i in tqdm(range(Y.shape[0]),desc=f"Removing interference with: {pargs.loss} loss; c={pargs.c}; tau={pargs.tau}"):
            ...
            res.append(map_help_fun(args))

    else:
        args = [(Y.iloc[i,:].values, mu, W, tau, c, pargs.loss, Y.index[i]) for i in range(Y.shape[0])]
        with mp.Pool(processes = num_cores) as pool:
            res = list(tqdm(pool.imap(map_help_fun, args),total=len(args),desc=f"Removing interference with: {pargs.loss} loss; c={pargs.c}; tau={pargs.tau}",leave=False,))
